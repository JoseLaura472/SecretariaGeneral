...
public interface ITipoResolucionDao extends CrudRepository<TipoResolucion, Long>{
    @Query(value = "select * from tipo_resolucion as tpr left join consejo as co ON tpr.id_consejo = co.id_consejo WHERE tpr.id_consejo=?1 AND tpr.estado_tipo_resolucion='A' ", nativeQuery = true)
